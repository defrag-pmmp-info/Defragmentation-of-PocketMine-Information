...
#### (作業中)エンティティネットワークメタデータの削除
- すべてのネットワークメタデータ関連の定数は`Entity`クラスから取り除かれプロトコル層に移動した。ネットワークメタデータをAPIから完全に取り除くことを意図しているがまだ完全にはできていない。
  - `Entity::DATA_FLAG_*`定数は`pocketmine\network\mcpe\protocol\types\entity\EntityMetadataFlags`に移動した
  - `Entity::DATA_TYPE_*`定数は`pocketmine\network\mcpe\protocol\types\entity\EntityMetadataTypes`に移動した
  - `Entity::DATA_*`定数は`pocketmine\network\mcpe\protocol\types\entity\EntityMetadataProperties`に移動した
- `DataPropertyManager`は`pocketmine\network\mcpe\protocol\types\entity` 名前空間に移動しAPIとは考えられなくなった。
- `Entity`の内部フックとして`syncNetworkData()`を導入する。 この関数はエンティティプロパティとエンティティネットワークデータセットを同期するためのものである。
- エンティティの内部プロパティを保持するためのネットワークメタデータセットの内部利用は取り除かれた。エンティティは通常のクラスプロパティを用いて、必要に応じてネットワークデータセットを同期するようにすべきである。
- `Entity->propertyManager`は`Entity->networkProperties`にリネーム
- `Entity->getDataPropertyManager()`は`Entity->getNetworkProperties()`にリネーム
